...
''', '''新竹上线时间'''], []]},
    ...
    def readReturnOrder_Update(self, team, filePath):
        ...
        if '吉客印新竹退款明细' in filePath or '现退明细' in filePath:
            ...
            new_path = "F:\\神龙签收率\\" + (datetime.datetime.now()).strftime('%m.%d') + "\\导状态\\{0} 导退货状态-临时2.xlsx".format(rq)
            print('正在输出 退货退款 导状态表…………')
            sql = '''SELECT 订单编号, 联系电话, 运单编号 AS 回执单号, 发货时间,  '退货退款' AS 状态,  4 AS 完成状态,  完成时间 AS 更新时间, 上线时间 
                    FROM {0}_return r 
