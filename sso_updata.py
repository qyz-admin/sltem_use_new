...
class Query_sso_updata(Settings):
    ...
    def order_getList(self, team, updata, begin, end, proxy_handle, proxy_id):  # 进入订单检索界面
        ...
        if max_count != 0 and max_count != []:
            ...
            try:
                ...
                print('转化失败： 更新-数据库总表', str(Exception) + str(e) + begin)
                sso = Settings_sso()
                sso.send_dingtalk_message( "https://oapi.dingtalk.com/robot/send?access_token=68eeb5baf4625d0748b15431800b185fec8056a3dbac2755457f3905b0c8ea1e", "总表-更新失败，请检查原因》》》本地数据库：：", ['18538110674'], "是")
        else:
            print('没有需要获取的信息！！！')
            return
        print('*' * 50)
    ...
    def gp_order(self, proxy_handle, proxy_id):
        ...
        sql = '''SELECT xj.订单编号, xj.下单时间, xj.新运单号 运单编号, xj.查件单号, xj.产品id, xj.商品名称, xj.下架时间, xj.仓库, xj.物流渠道, xj.币种, xj.统计时间, xj.记录时间, b.物流状态, c.标准物流状态,b.状态时间, NULL 系统订单状态, NULL 系统物流状态, 
                        IF(统计时间 >=CURDATE() ,'未发货',NULL) AS 状态, g.工单类型, g.工单是否完成,g.提交时间, g.登记人, g.运单编号 AS 运单号, IF(LENGTH(xj.新运单号) >=20,'是',NULL) 导新运单号
                FROM ( SELECT *
                       FROM 已下架表 x
