    ...
    def replaceSqlHost(self, team, query):
        if team in ('gat', 'slgat', 'slgat_hfh', 'slgat_hs'):
            sql = '''SELECT EXTRACT(YEAR_MONTH  FROM h.下单时间) 年月,
            				        ...
            				        h.联系电话 电话号码,
            				        h.运单号 运单编号,
            				        IF(h.物流渠道 LIKE "台湾-天马-711" AND LENGTH(h.运单号)=20, CONCAT(861,RIGHT(h.运单号,8)), IF((h.logisticsName LIKE "台湾-速派-新竹改派" or h.logisticsName LIKE "台湾-易速配-新竹改派") AND h.运单号 LIKE "A%",RIGHT(h.运单号,LENGTH(h.运单号)-1),UPPER(h.运单号))) 查件单号,
            				        h.订单状态 系统订单状态,
