...
class QueryTwo(Settings, Settings_sso):
    ...
    def outport_getDeliveryList(self, timeStart, timeEnd, logisticsN_begin, logisticsN_end):
        ...
        sql8 = '''SELECT s2.派送类型, s2.月份, s2.总订单,
                        ...
                        FROM( SELECT s1.月份, s1.派送类型, COUNT(ss3.订单编号) AS 总订单,
                            ...
                            FROM (  SELECT *,EXTRACT(YEAR_MONTH FROM 创建日期) AS 月份,
                                            IF(w.订单号 IS NOT NULL,
        										...
        										IF(派送问题 = '送至便利店' OR 派送问题 = '客户自取','送至便利店',IF(派送问题 = '客户长期不在' OR 派送问题 = '送达客户不在','送达客户不在',派送问题))) AS 旧版派送类型,
        									IF(w.订单号 IS NOT NULL,
                                            ...
                                            IF(w.订单号 IS NOT NULL,
                                                IF(EXTRACT(YEAR_MONTH FROM 创建日期) <> '202304',
                                                        IF(w.外呼类型 ='到站自取','营业所自取带挂机短信',IF(w.外呼类型 ='预约时间配送','预约时间配送不带挂机短信',
                                                        IF(w.外呼类型 ='超商' AND 是否短信 ='否','送至便利店不带挂机短信',IF(w.外呼类型 ='超商' AND 是否短信 ='是','送至便利店带挂机短信',派送问题)))),
                                                        IF(w.外呼类型 ='超商' AND 是否短信 ='否','送至便利店不带挂机短信',IF(派送问题 = '送至便利店' OR 派送问题 = '客户自取','送至便利店',IF(派送问题 = '客户长期不在' OR 派送问题 = '送达客户不在','送达客户不在',派送问题)))),
                                                IF(ms.短信订单号 IS NOT NULL,IF(派送问题 = '送至便利店' OR 派送问题 = '客户自取','送至便利店-发短信',IF(派送问题 = '客户长期不在' OR 派送问题 = '送达客户不在','送达客户不在-发短信',CONCAT(派送问题,'-发短信'))),
                                                    IF(派送问题 = '送至便利店' OR 派送问题 = '客户自取','送至便利店',IF(派送问题 = '客户长期不在' OR 派送问题 = '送达客户不在','送达客户不在',派送问题)))
                                                ) AS 派送类型
                                    FROM 派送问题件_跟进表 g
        							...
        							LEFT JOIN 
                                    (  SELECT 订单编号 AS 短信订单号
                                            FROM 短信日志_发送时间 m
                                            WHERE id IN  (SELECT MAX(id) 
                                                         FROM 短信日志_发送时间 m
                                                         WHERE m.`发送时间` >= DATE_FORMAT(DATE_SUB(curdate(), INTERVAL 13 MONTH),'%Y-%m-%d') AND m.短信用途 LIKE '%派送问题%'
                                                         GROUP BY 订单编号
                                                         )
                                    ) ms ON g.订单编号 = ms.短信订单号
                        ...
                        ORDER BY 
                        FIELD(派送类型,'营业所自取带挂机短信','预约时间配送不带挂机短信','送至便利店带挂机短信','送至便利店不带挂机短信',
									 '送至便利店','送至便利店-发短信','客户要求更改派送时间或者地址','客户要求更改派送时间或者地址-发短信',
									 '客户不接电话','客户不接电话-发短信','地址问题','地址问题-发短信','送达客户不在','送达客户不在-发短信','拒收-发短信','拒收','合计'),
        ...
        sql = '''SELECT 币种, EXTRACT(YEAR_MONTH FROM 日期) AS 月份, 日期 AS 创建日期, 
                        CASE DATE_FORMAT(日期,'%w')	WHEN 1 THEN '星期一' WHEN 2 THEN '星期二' WHEN 3 THEN '星期三' WHEN 4 THEN '星期四' WHEN 5 THEN '星期五' WHEN 6 THEN '星期六' WHEN 0 THEN '星期日' END as 月周,
            ...
            FROM ( SELECT s1.币种, s1.日期, s1.签收单量, s1.拒收单量, s1.总单量,  派送问题件单量, 问题件类型,
					...
					( SELECT 币种, 创建日期, COUNT(订单编号) AS 派送问题件单量,
                     ...
                     LEFT JOIN 
                     ( SELECT 币种, 创建日期, CASE DATE_FORMAT(创建日期,'%w')	WHEN 1 THEN '星期一' WHEN 2 THEN '星期二' WHEN 3 THEN '星期三' WHEN 4 THEN '星期四' WHEN 5 THEN '星期五' WHEN 6 THEN '星期六' WHEN 0 THEN '星期日' END as 上月周, COUNT(订单编号) AS 上月派送问题件单量
                                        FROM 派送问题件_跟进表 p
        ...
        df.to_sql('tem', con=self.engine1, index=False, if_exists='replace')
...
if __name__ == '__main__':
    ...
    if int(select) == 99:
        ...
        if int(select) == 11:
            ...
            m.outport_getDeliveryList(timeStart, timeEnd, logisticsN_begin, logisticsN_end)
        ...
        logisticsN_begin = '2022-07-11'                             # 送达客户不在/客户长期不在  物流轨迹查询时间
        logisticsN_end = '2022-07-31'
        m.outport_getDeliveryList('2023-03-01', '2023-03-23', logisticsN_begin, logisticsN_end)
