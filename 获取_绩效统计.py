...
class QueryOrder_Code(Settings, Settings_sso):
                ...
                FROM ( SELECT 代下单客服, count(订单编号) as 总计,
                        ...
                        FROM (
                                (	SELECT '促单' as 类型, 代下单客服, 订单编号, 订单状态,物流状态, 
                                    ...
                                    FROM 促单_下单时间 s1
                                    WHERE  s1.代下单客服 in ({0}}) AND s1.克隆人 = '' AND s1.`统计月份` = '{0}' AND DATE_FORMAT( s1.`记录时间`, '%Y-%m-%d') = '{1}'
                                )
                                UNION
                                (	SELECT '挽单列表' as 类型, 创建人, 订单编号,  当前订单状态, 当前物流状态, 
                                    ...
                                    FROM 挽单列表_创建时间 w
                                    WHERE  w.`统计月份` = '{0}' and DATE_FORMAT(w.`记录时间`, '%Y-%m-%d') = '{1}' AND w.删除人 = '' AND w.挽单类型 IN ("取消挽单","未支付/支付失败挽单")
                                )
                        ) s1
                        GROUP BY  代下单客服
        ...
        sql32 = '''SELECT 最新客服处理人,
			    ...
			    FROM (
						(	SELECT '客诉退货' as 类型, 最新客服处理人, 订单编号, 赠品补发订单编号, 订单状态, 物流状态, 赠品补发订单状态, 赠品补发物流状态,
										...
										IF(赠品补发物流状态 IN ('已退货','拒收', '自发头程丢件', '客户取消'), 赠品补发物流状态, IF(赠品补发物流状态 IN ('已签收','理赔'), IF(赠品补发订单状态 = '已退货(销售)','拒收',赠品补发物流状态), 
											IF(赠品补发物流状态 = '发货中','在途', 
											IF(赠品补发物流状态 = '' or 赠品补发物流状态 IS NULL or 赠品补发物流状态 = '暂无物流状态', IF(赠品补发订单状态 IN ('已删除','未支付','支付失败'),'无效订单','未发货'),赠品补发物流状态)))) as 赠品补发最终状态, 统计月份
							FROM (	SELECT *, IF(赠品补发订单编号 <> "",IF(最新客服处理结果 LIKE '%补发海外仓%','统计','不统计'),'不统计') AS 是否统计	
									FROM 物流客诉件_创建时间 k1
									WHERE  k1.`统计月份` = '{0}' and DATE_FORMAT(k1.`记录时间`,'%Y-%m-%d') = '{1}' AND k1.最新处理状态 <> ""
							) k	
							WHERE  是否统计 = "统计"
						)
						UNION
						(	SELECT '挽单列表' as 类型, 创建人, 订单编号, NULL AS 赠品补发订单编号, 当前订单状态, 当前物流状态, NULL AS 赠品补发订单状态,	NULL AS 赠品补发物流状态,
							...
							FROM 挽单列表_创建时间 w
							WHERE  w.`统计月份` = '{0}' and DATE_FORMAT(w.`记录时间`, '%Y-%m-%d') = '{1}' AND w.删除人 = '' AND w.挽单类型 IN ("退换补挽单")
						)
				) k	
    ...
    def service_check2(self, username_Cudan, username_Jushou, month_time, day_time):
        ...
        sql11 = '''SELECT *, IF(当前物流状态 IN ('已退货','拒收', '自发头程丢件', '客户取消'), 当前物流状态,
                                IF(当前物流状态 IN ('已签收','理赔'), IF(当前订单状态 = '已退货(销售)','拒收',当前物流状态), IF(当前物流状态 = '发货中','在途',
                                IF(当前物流状态 = '' or 当前物流状态 IS NULL or 当前物流状态 = '暂无物流状态', IF(当前订单状态 IN ('已删除','未支付','支付失败'),'无效订单','未发货'),当前物流状态)))) as 最终状态
                FROM 挽单列表_创建时间 s1
                WHERE  s1.`统计月份` = '{0}' and DATE_FORMAT(s1.`记录时间`,'%Y-%m-%d') = '{1}';'''.format(month_time, day_time)
        df11 = pd.read_sql_query(sql=sql11, con=self.engine1)
        ...
        sql = '''REPLACE INTO {0}(类型, 客服处理人, 订单编号, 赠品补发订单编号,订单状态,物流状态, 赠品补发订单状态, 赠品补发物流状态,最终状态, 赠品补发最终状态,统计月份, 是否计算, 计算月份, 更新月份, 记录时间, 更新时间) 
                   SELECT 挽单类型 as 类型, 
                            创建人 as 客服处理人, 订单编号, 赠品补发订单编号,
                      当前订单状态 as 订单状态,
                      当前物流状态 as 物流状态, 赠品补发订单状态, 赠品补发物流状态,最终状态, 赠品补发最终状态, 统计月份,
                      IF(最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),'是','否') as 是否计算, 
                      IF(最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"), 统计月份, '-') as 计算月份,  
                      DATE_FORMAT(curdate(),'%Y%m') as 更新月份, 记录时间, NOW() 更新时间
                    FROM cache_ch 
        ...
        sql = '''REPLACE INTO {0}(类型, 新单克隆人, 订单编号, 克隆后新订单号,新单订单状态,新单物流状态, 最终状态, 赠品补发最终状态,统计月份, 是否计算, 计算月份, 更新月份, 记录时间, 更新时间) 
               SELECT 挽单类型 as 类型, 
                        创建人 as 新单克隆人, 订单编号, 
                  null as 克隆后新订单号,
                  当前订单状态 as 新单订单状态,
                  当前物流状态 as 新单物流状态,  最终状态, 赠品补发最终状态,统计月份,
                  IF(最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),'是','否') as 是否计算, 
                  IF(最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"), 统计月份, '-') as 计算月份,  
                  DATE_FORMAT(curdate(),'%Y%m') as 更新月份, 记录时间, NOW() 更新时间
                FROM cache_ch 
        ...
        sql = '''SELECT '采购异常' AS 类型, 客服处理人, 订单编号, 订单状态, 物流状态, 
                ...
                WHERE  s1.`统计月份` = '{0}' and DATE_FORMAT(s1.`记录时间`,'%Y-%m-%d') = '{1}' and s1.客服处理人 in ({2})
                  AND (s1.`客服处理结果` NOT LIKE '%已发货%' OR s1.`客服处理结果` NOT LIKE '%改派%' OR s1.`客服处理结果` NOT LIKE '%无须处理%') ;'''.format(month_time, day_time, username_caigou_yadan_wentijian)
        df = pd.read_sql_query(sql=sql, con=self.engine1)
        ...
        sql = '''SELECT '压单' AS 类型, 最新客服处理人 AS 客服处理人, 订单编号, 订单状态, 物流状态, 
                ...
                WHERE s1.`统计月份` = '{0}' and DATE_FORMAT(s1.`记录时间`,'%Y-%m-%d') = '{1}' AND s1.最新处理结果 <> "" and s1.最新客服处理人 in ({2})
                 AND (s1.最新处理结果 NOT LIKE '%无人接听%' AND s1.最新处理结果 NOT LIKE '%电话暂停使用%' AND s1.最新处理结果 NOT LIKE '%无效号码%' AND s1.最新处理结果 NOT LIKE '%电话停止使用%');'''.format(month_time, day_time, username_caigou_yadan_wentijian)
        df = pd.read_sql_query(sql=sql, con=self.engine1)
        df.to_sql('cache_ch', con=self.engine1, index=False, if_exists='replace')
        sql = '''REPLACE INTO {0}(类型, 客服处理人, 订单编号, 订单状态,物流状态, 最终状态, 是否计算, 统计月份, 计算月份, 更新月份, 记录时间, 更新时间) 
                           SELECT 类型, 客服处理人, 订单编号, 订单状态,物流状态, 最终状态,
        ...
        sql = '''SELECT '问题订单' AS 类型, 转化人 AS 客服处理人, 订单编号, 系统订单状态 AS 订单状态, 系统物流状态 AS 物流状态, 
                        IF(系统物流状态 IN ('已退货','拒收', '自发头程丢件', '客户取消'), 系统物流状态, IF(系统物流状态 IN ('已签收','理赔'), IF(订单状态 = '已退货(销售)','拒收',系统物流状态), IF(系统物流状态 = '发货中','在途',
			            IF(系统物流状态 = '' or 系统物流状态 IS NULL or 系统物流状态 = '暂无物流状态', IF(订单状态 IN ('已删除','未支付','支付失败'),'无效订单','未发货'),系统物流状态)))) as 最终状态, 统计月份, 记录时间
                FROM 系统问题件_下单时间 s1
        ...
        sql = '''SELECT '派送问题件' AS 类型, 最新处理人 AS 客服处理人, 订单编号, 订单状态, 物流状态, 
			            ...
			            IF(物流状态 = '' or 物流状态 IS NULL or 物流状态 = '暂无物流状态', IF(订单状态 IN ('已删除','未支付','支付失败'),'无效订单','未发货'),物流状态)))) as 最终状态, 统计月份, 记录时间
                FROM (
                        SELECT *, IF(最新客服处理 LIKE '已处理%' OR 最新客服处理 LIKE '货件拒收%' OR 最新客服处理 LIKE '货态拒收%' OR 最新客服处理 LIKE '货态签收%' OR 最新客服处理 LIKE '货态已签收%' 
                                OR 最新客服处理 LIKE '已通知客户%'  OR 最新客服处理 LIKE '已告知客户%' OR 最新客服处理 LIKE '通知客户取货%' OR 最新客服处理 LIKE '通知客户自取%' OR 最新客服处理 LIKE '请通知客户取货%' 
                                OR 最新客服处理 LIKE '%暂停使用%' OR 最新客服处理 LIKE '%停止使用%'  OR 最新客服处理 LIKE '%没有登记%' OR 最新客服处理 LIKE '%无登记%' OR 最新客服处理 LIKE '%电话停机%' OR 最新客服处理 LIKE '已签收' 
                                OR 最新客服处理 LIKE '以帮客户下单' OR 最新客服处理 LIKE '已发图片' OR 最新客服处理 LIKE '无人额急停%' OR 最新客服处理 LIKE '无人接听%' OR 最新客服处理 LIKE '无效号码%','不统计', 
                                IF(最新客服处理 NOT LIKE '%拒收%',
        ...
        sql = '''SELECT 新单克隆人,
                      ...
                      SUM(IF(最终状态 = '在途',1,0)) AS 在途,
                      SUM(IF(最终状态 IN ('已签收','拒收','已退货','理赔','自发头程丢件'),1,0)) AS 已完成,
                      SUM(IF(最终状态 <> '无效订单',1,0)) AS 有效单量,
                      COUNT(订单编号) AS 总计,
                      concat(ROUND(IFNULL(SUM(IF(最终状态 = '已签收',1,0)) / SUM(IF(最终状态 IN ('已签收','拒收','已退货','理赔','自发头程丢件'),1,0)),0) * 100,2),'%') as 签收率,
                      concat(ROUND(IFNULL(SUM(IF(最终状态 IN ('已签收','拒收','已退货','理赔','自发头程丢件'),1,0)) / COUNT(订单编号),0) * 100,2),'%') as 完成占比,
                      SUM(IF(最终状态 = '无效订单',1,0)) AS 无效订单,
                      concat(ROUND(IFNULL(SUM(IF(最终状态 <> '无效订单',1,0)) / COUNT(订单编号),0) * 100,2),'%') as 转化率
                FROM 拒收挽单_挽单列表_计算统计 j
                WHERE j.`更新月份` = '202303'
                GROUP BY 新单克隆人
    ...
    def service_check22(self):
        ...
        sql = '''SELECT '问题订单' AS 类型, 转化人 AS 客服处理人, 订单编号, 系统订单状态 AS 订单状态, 系统物流状态 AS 物流状态, 
                        ...
                        IF(系统物流状态 IN ('已签收','理赔'), IF(订单状态 = '已退货(销售)','拒收',系统物流状态), IF(系统物流状态 = '发货中','在途',
			            IF(系统物流状态 = '' or 系统物流状态 IS NULL or 系统物流状态 = '暂无物流状态', IF(订单状态 IN ('已删除','未支付','支付失败'),'无效订单','未发货'),系统物流状态)))) as 最终状态, 统计月份, 记录时间
                FROM 系统问题件_下单时间 s1
        ...
        sql = '''SELECT '客诉件' AS 类型, 最新客服处理人 AS 客服处理人, 订单编号, 订单状态, 物流状态, 
                        IF(物流状态 IN ('已退货','拒收', '自发头程丢件', '客户取消'), 物流状态,
                        IF(物流状态 IN ('已签收','理赔'), IF(订单状态 = '已退货(销售)','拒收',物流状态), IF(物流状态 = '发货中','在途',
			            IF(物流状态 = '' or 物流状态 IS NULL or 物流状态 = '暂未物流状态', IF(订单状态 IN ('已删除','未支付','支付失败'),'无效订单','未发货'),物流状态)))) as 最终状态, 统计月份, 记录时间
                FROM (
                        SELECT *, IF(赠品补发订单编号 <> "",IF(最新客服处理结果 LIKE '%补发海外仓%','统计','不统计'),'不统计') AS 是否统计	
                        FROM 物流客诉件_创建时间 s1
                        WHERE  s1.`统计月份` = DATE_FORMAT(DATE_SUB(curdate(), INTERVAL 1 MONTH),'%Y%m') and DATE_FORMAT(s1.`记录时间`,'%Y%m%d') = DATE_FORMAT('2023-03-06','%Y%m%d') AND s1.最新处理状态 <> "" 
        ...
        sql = '''SELECT '拒收件' AS 类型, 新单克隆人, 订单编号, 再次克隆下单 AS 克隆后新订单号, 新单订单状态, 新单物流状态, 
			            ...
			            IF(新单物流状态 = '' or 新单物流状态 IS NULL or 新单物流状态 = '暂未物流状态', IF(订单状态 IN ('已删除','未支付','支付失败'),'无效订单','在途'),新单物流状态)))) as 最终状态, 统计月份, 记录时间
                FROM (
                        SELECT *
                        FROM 拒收问题件_跟进时间 s1
