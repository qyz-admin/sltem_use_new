...
class MysqlControl(Settings):
    ...
    def creatMyOrderSl(self, team):     # 最近五天的全部订单信息
        ...
        if team == 'sltg' or team == 'slxmt' or team == 'slrb' or team == 'slgat':
            ...
            # 11-12月的
            begin = datetime.date(2021, 1, 27)
            print(begin)
            end = datetime.date(2021, 2, 20)
            print(end)
        for i in range((end - begin).days):     # 按天循环获取订单状态
            day = begin + datetime.timedelta(days=i)
            # print(str(day))
    ...
    def connectOrder(self, team):
        ...
        if team == 'slgat0':
            df.to_sql('d1_slgat', con=self.engine1, index=False, if_exists='replace')
        elif team == 'slxmt0':
            df.to_sql('d1_slxmt', con=self.engine1, index=False, if_exists='replace')
        elif team == 'sltg0':
            df.to_sql('d1_sltg', con=self.engine1, index=False, if_exists='replace')
        elif team == 'slrb0':
            df.to_sql('d1_slrb', con=self.engine1, index=False, if_exists='replace')
        else:
        ...
        for tem in match[team]:
            ...
            sql = '''SELECT 年月,币种,物流方式,IF(s.天数=90,NULL,s.天数) AS 天数,总计 ,签收量,完成量,签收率完成,签收率总计,累计完成占比
                    FROM (SELECT IFNULL(年月,'总计') AS 年月,
                        ...
                        with rollup
                    ) s WHERE s.`币种` != '总计'  AND s.`年月` != '总计';'''.format(tem)
            df = pd.read_sql_query(sql=sql, con=self.engine1)
            listT.append(df)
