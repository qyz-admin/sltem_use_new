        ...
        if week.isoweekday() == 2:
            ...
            sql = '''SELECT *
					FROM(SELECT	IFNULL(月份, '总计') 月份, IFNULL(地区, '总计') 地区, IFNULL(产品id, '总计') 产品id, IFNULL(产品名称, '总计') 产品名称,
							...
							SUM(神龙主页运营单量) 神龙主页运营单量, 
                                concat(ROUND(SUM(神龙主页运营签收) / SUM(神龙主页运营总量) * 100,2),'%') as 神龙主页运营总计签收,
                                concat(ROUND(SUM(神龙主页运营完成) / SUM(神龙主页运营总量) * 100,2),'%') as 神龙主页运营完成占比					
                        FROM(SELECT 年月 月份,币种 地区, 产品id, 产品名称,
                                    ...
                                    SUM(IF(所属团队 = '神龙神龙主页运营',1,0)) as 神龙主页运营单量,
									SUM(IF(所属团队 = '神龙神龙主页运营',价格,0)) as 神龙主页运营总量,
                                    SUM(IF(所属团队 = '神龙主页运营' AND 最终状态 = "已签收",价格,0)) as 神龙主页运营签收,
                                    SUM(IF(所属团队 = '神龙主页运营' AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),价格,0)) as 神龙主页运营完成,
...
--                   WITH ROLLUP 
    ...
    def qsb_new(self, team, month_last):  # 报表各团队近两个月的物流数据
        ...
        sql41 = '''SELECT *
            FROM(SELECT IFNULL(月份, '总计') 月份,IFNULL(地区, '总计') 地区,IFNULL(是否改派, '总计') 是否改派,IFNULL(物流方式, '总计') 物流方式,IFNULL(家族, '总计') 家族,									
                        ...
                        concat(ROUND(SUM(改派单量) / SUM(总单量) * 100,2),'%') as 改派占比,
                            ...
                            concat(ROUND(SUM(改派签收) / SUM(改派单量) * 100,2),'%') as 改派总计签收,
                            concat(ROUND(SUM(改派完成) / SUM(改派单量) * 100,2),'%') as 改派完成占比
                    FROM(SELECT 年月 月份, 币种 地区, 是否改派, 物流方式, 所属团队 as 家族,
                                COUNT(订单编号) as 总单量,
								SUM(IF(最终状态 = "已签收",1,0)) 签收,
								SUM(IF(最终状态 IN ("已签收","拒收","已退货","理赔", "自发头程丢件"),1,0)) 完成,
								SUM(IF(最终状态 = "已退货",1,0)) 退货,
        ...
        print(df51.columns)
        df51.rename(columns={'11总单量': '{0}' + '总单量', '11总计签收': '{0}' + '总计签收', '11完成签收': '{0}' + '完成签收', '11完成占比': '{0}' + '完成占比',
                             '22总单量': '{1}' + '总单量', '22总计签收': '{1}' + '总计签收', '22完成签收': '{1}' + '完成签收', '22完成占比': '{1}' + '完成占比',
                             '33总单量': '{2}' + '总单量', '33总计签收': '{2}' + '总计签收', '33完成签收': '{2}' + '完成签收', '33完成占比': '{2}' + '完成占比',
                             '44总单量': '{3}' + '总单量', '44总计签收': '{3}' + '总计签收', '44完成签收': '{3}' + '完成签收', '44完成占比': '{3}' + '完成占比',
                             '55总单量': '{4}' + '总单量', '55总计签收': '{4}' + '总计签收', '55完成签收': '{4}' + '完成签收', '55完成占比': '{4}' + '完成占比',
                             '66总单量': '{5}' + '总单量', '66总计签收': '{5}' + '总计签收', '66完成签收': '{5}' + '完成签收', '66完成占比': '{5}' + '完成占比',
                             '77总单量': '{6}' + '总单量', '77总计签收': '{6}' + '总计签收', '77完成签收': '{6}' + '完成签收', '77完成占比': '{6}' + '完成占比',
                             '88总单量': '{7}' + '总单量', '88总计签收': '{7}' + '总计签收', '88完成签收': '{7}' + '完成签收', '88完成占比': '{7}' + '完成占比',
                             '99总单量': '{8}' + '总单量', '99总计签收': '{8}' + '总计签收', '99完成签收': '{8}' + '完成签收', '99完成占比': '{8}' + '完成占比',
                             '100总单量': '{9}' + '总单量', '100总计签收': '{9}' + '总计签收', '100完成签收': '{9}' + '完成签收', '100完成占比': '{9}' + '完成占比',
                             '110总单量': '{10}' + '总单量', '110总计签收': '{10}' + '总计签收', '110完成签收': '{10}' + '完成签收', '110完成占比': '{10}' + '完成占比',
                             '120总单量': '{11}' + '总单量', '120总计签收': '{11}' + '总计签收', '120完成签收': '{11}' + '完成签收', '120完成占比': '{11}' + '完成占比',
                             '130总单量': '{12}' + '总单量', '130总计签收': '{12}' + '总计签收', '130完成签收': '{12}' + '完成签收', '130完成占比': '{12}' + '完成占比'
                             }, inplace=True).format(t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13)
        print(df51.columns)
        listT.append(df51)
