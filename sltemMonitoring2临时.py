...
class SltemMonitoring(Settings):
    ...
    def order_Monitoring(self, team):
        ...
        if match[team] == 'gat':
            sql = '''SELECT LEFT(年月,4) AS 年, 年月, 旬, 日期, 所属团队 as 团队, 币种, 订单来源, 订单编号, 出货时间, 状态时间, 上线时间, 最终状态, 是否改派,物流方式,产品id,
    ...
    def sl_Monitoring(self, team, now_month, last_month, ready):
        ...
        sqltime82 = '''SELECT s.币种, s.年月, IF(s.旬 =1,'上旬',IF(s.旬 =2,'中旬',IF(s.旬 =3,'下旬',s.旬))) AS 旬, s.是否改派, s.物流方式,
                        ...
                        ORDER BY FIELD(s.`币种`,'合计'), 
                                ...
                                FIELD(s.`是否改派`,'改派','直发','合计'),
                                FIELD(s.`物流方式`, {7},"合计")
                        ;'''.format(family, now_month, now_month_old, last_month, last_month_old, currency, match[team],self.logistics_name)
        listT.append(sqltime82)
