    ...
    def update_logistics_name(self):
        sql = '''UPDATE gat_zqsb d
                SET d.`物流方式`= IF(d.`物流方式` LIKE '香港-易速配-顺丰%','香港-易速配-顺丰', 
                                ...
                                IF(d.`物流方式` LIKE '%铱熙无敌-新竹%' or d.`物流方式` LIKE '%优美宇通-新竹%','台湾-铱熙无敌-新竹', 
        ...
        sql = '''UPDATE gat_zqsb d
                SET d.`物流方式`= IF(d.`物流方式` LIKE '香港-森鸿%','香港-森鸿-改派',
                                ...
                                IF(d.`物流方式` LIKE '台湾-优美宇通-新竹%','台湾-铱熙无敌-新竹改派',d.`物流方式`))))))))))))))
                WHERE d.`是否改派` ='改派';'''
        print('正在修改-改派的物流渠道…………')
        pd.read_sql_query(sql=sql, con=self.engine1, chunksize=10000)
...
--                   WITH ROLLUP 
    ...
    def gat_new(self, team, month_last, month_yesterday, currency_id):  # 报表各团队近两个月的物流数据
        ...
        sql0 = '''SELECT s2.家族,s2.币种,s2.年月,s2.是否改派,s2.物流方式,
                ...
                HAVING s2.年月 <> '合计'
    ORDER BY FIELD(s2.`家族`,'神龙','火凤凰','神龙运营1组','Line运营','金蝉家族','APP运营','神龙主页运营','小虎队','红杉','金狮','合计'),
        ...
        ORDER BY FIELD(s.`家族`,'神龙','火凤凰','神龙运营1组','Line运营','金蝉家族','APP运营','神龙主页运营','小虎队','红杉','金狮','合计'),
                FIELD(s.`地区`,'台湾','香港','合计'),
    ...
    def qsb_new(self, team, month_last):  # 报表各团队近两个月的物流数据
        month_now = datetime.datetime.now().strftime('%Y-%m-%d')
        match = {'gat': '港台-每日'}
        not_team = '"红杉家族-港澳台", "APP运营", "红杉家族-港澳台2", "金狮-港澳台", "金鹏家族-小虎队","Line运营","神龙-主页运营","奥创队"'
        # if team == 'ga9t':
        #     month_last = (datetime.datetime.now().replace(day=1) - datetime.timedelta(days=1)).strftime('%Y-%m') + '-01'
        #     month_now = datetime.datetime.now().strftime('%Y-%m-%d')
        # else:
