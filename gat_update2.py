...
class QueryUpdate(Settings):
    ...
    def gat_new(self, team, month_last, month_yesterday):  # 报表各团队近两个月的物流数据
        ...
        sql0 = '''SELECT s2.家族,s2.币种,s2.年月,s2.是否改派,s2.物流方式,
                ...
                HAVING s2.年月 <> '合计'
    ORDER BY FIELD(s2.`家族`,'神龙','火凤凰','小虎队','神龙-低价','神龙-主页运营1组','红杉','金狮','合计'),
        ...
        ORDER BY FIELD(s.`家族`,'神龙','火凤凰','小虎队','神龙-低价','神龙-主页运营1组','红杉','金狮','合计'),
                FIELD(s.`地区`,'台湾','香港','合计'),
    ...
    def qsb_new(self, team, month_last):  # 报表各团队近两个月的物流数据
        ...
        #                 ORDER BY 月份 DESC,
        #                         FIELD( 地区, '台湾', '香港', '总计' ),
        #                         FIELD( 家族, '神龙', '火凤凰', '金狮', '金鹏', '神龙-低价','神龙-主页运营1组', '红杉', '总计');'''.format(month_last, team)
        sql0 = '''SELECT *
                ...
                ORDER BY 月份 DESC,
                        FIELD( 地区, '台湾', '香港', '总计' ),
                        FIELD( 家族, '神龙', '火凤凰', '小虎队', '神龙-低价', '神龙-主页运营1组', '红杉', '金狮', '总计'),
        ...
        sql51 = '''SELECT *
                ...
                ORDER BY FIELD(ss.`家族`,'神龙','火凤凰','小虎队','神龙-低价', '神龙-主页运营1组','红杉','金狮','总计'),
        ...
        sql20 = '''SELECT *
                    ...
                    ORDER BY 月份 DESC,
                            FIELD( 地区, '台湾', '香港', '总计' ),
                            FIELD( s.家族, '神龙','火凤凰','小虎队','神龙-低价', '神龙-主页运营1组','红杉','金狮', '总计' ),
        ...
        sql0 = '''SELECT s2.家族,s2.币种,s2.年月,s2.是否改派,s2.省洲,
                    ...
                    FROM ( SELECT IFNULL(s1.币种,'合计') as 币种,
                        ...
                        FROM (SELECT cx.币种 as 币种,cx.家族 as 家族,cx.年月 as 年月,cx.是否改派 as 是否改派,cx.省洲 as 省洲,
                                    ...
                                    SUM(`价格RMB`) - SUM(IF(最终状态 = "未发货",`价格RMB`,0)) as 发货金额
                                FROM (SELECT *,
                                        IF(cc.团队 LIKE "%红杉%","红杉",IF(cc.团队 LIKE "%火凤凰%","火凤凰",IF(cc.团队 LIKE "%神龙%","神龙",IF(cc.团队 LIKE "%金狮%","金狮",IF(cc.团队 LIKE "%金鹏%","小虎队",cc.团队))))) as 家族
                                    FROM {0}_zqsb cc where cc.`运单编号` is not null AND cc.日期 >= '{1}' AND cc.日期 <= '{2}'
                                ) cx
                    ...
                    HAVING s2.年月 <> '合计'
        ORDER BY FIELD(s2.`家族`,'神龙','火凤凰','小虎队','红杉','神龙-主页运营1组','金狮','合计'),
                FIELD(s2.`币种`,'台湾','香港','合计'),
                s2.`年月`,
                FIELD(s2.`是否改派`,'改派','直发','合计'),
