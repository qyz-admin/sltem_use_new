        ...
        if week.isoweekday() == 0 or week.isoweekday() == 0:
            ...
            sql = '''SELECT 团队 AS 家族, 币种, 产品id, 产品名称, 
                    ...
                    FROM(SELECT s.团队, s.币种, s.产品id, s.产品名称, 
			                    ...
			                    IF(s.团队 = '神龙家族-台湾' AND s.币种 = '台湾',(avg_sign_rate-0.825),
			                    IF(s.团队 = '神龙家族-台湾' AND s.币种 = '香港',(avg_sign_rate-0.89) ,
			                    IF(s.团队 = '火凤凰-台湾' AND s.币种 = '台湾',(avg_sign_rate-0.87),
			                    IF(s.团队 = '火凤凰-台湾' AND s.币种 = '香港',(avg_sign_rate-0.89), 
			                    IF(s.团队 = '神龙-香港' AND s.币种 = '台湾',(avg_sign_rate-0.86), 
			                    IF(s.团队 = '神龙-香港' AND s.币种 = '香港',(avg_sign_rate-0.88), NULL)))))) AS 历史平均影响, 
			                    sign_rate AS 预测, 
			                    IF(s.团队 = '神龙家族-台湾' AND s.币种 = '台湾',(sign_rate-0.825),
			                    IF(s.团队 = '神龙家族-台湾' AND s.币种 = '香港',(sign_rate-0.89) ,
			                    IF(s.团队 = '火凤凰-台湾' AND s.币种 = '台湾',(sign_rate-0.87),
			                    IF(s.团队 = '火凤凰-台湾' AND s.币种 = '香港',(sign_rate-0.89), 
			                    IF(s.团队 = '神龙-香港' AND s.币种 = '台湾',(sign_rate-0.86), 
			                    IF(s.团队 = '神龙-香港' AND s.币种 = '香港',(sign_rate-0.88), NULL)))))) AS 预测影响,
			                    IF(s.团队 = '神龙家族-台湾' AND s.币种 = '台湾',0.825,
			                    IF(s.团队 = '神龙家族-台湾' AND s.币种 = '香港',0.89,
			                    IF(s.团队 = '火凤凰-台湾' AND s.币种 = '台湾',0.87,
			                    IF(s.团队 = '火凤凰-台湾' AND s.币种 = '香港',0.89, 
			                    IF(s.团队 = '神龙-香港' AND s.币种 = '台湾',0.86, 
			                    IF(s.团队 = '神龙-香港' AND s.币种 = '香港',0.88, NULL)))))) AS 目标签收率
...
--                   WITH ROLLUP 
    ...
    def gat_new(self, team, month_last, month_yesterday, currency_id):  # 报表各团队近两个月的物流数据
        ...
        logistics_name = '''
                        ...
                        '''
        team_name = '''IF(cc.团队 LIKE "神龙家族-台湾%","神龙",
                        IF(cc.团队 LIKE "火凤凰-台湾%","火凤凰", 
                        IF(cc.团队 LIKE "神龙-香港%","神龙香港",
                        IF(cc.团队 LIKE "火凤凰-香港%","火凤凰香港", 
                        IF(cc.团队 LIKE "金狮-港澳台%","金狮", 
                        IF(cc.团队 LIKE "红杉家族-港澳台%","红杉", 
                        IF(cc.团队 LIKE "金鹏家族-4组%","小虎队", 
                        IF(cc.团队 LIKE "神龙-主页运营%","神龙主页运营", cc.团队)))))))) as 家族
                    '''
        team_name2 = '''
                        "神龙","火凤凰","金蝉家族优化组","金蝉项目组","金蝉家族公共团队","神龙香港","火凤凰香港","Line运营","APP运营","客服中心-港台""研发部-研发团队","神龙主页运营","奥创队","小虎队","红杉","金狮","合计"
                    '''
        print('正在修改-港澳台-物流渠道…………')
        self.update_logistics_name()
        ...
        sql11 = '''SELECT s2.家族,s2.币种,s2.年月,s2.是否改派,s2.物流方式,s2.旬,
                ...
                FROM (SELECT IFNULL(s1.币种,'合计') as 币种,
                    ...
                    FROM (SELECT cx.币种 as 币种,cx.家族 as 家族,cx.年月 as 年月,cx.是否改派 as 是否改派,cx.物流方式 as 物流方式,
                                ...
                                SUM(`价格RMB`) - SUM(IF(最终状态 = "未发货",`价格RMB`,0)) as 发货金额
                            FROM (SELECT *, {4}
                                    FROM {0}_zqsb cc where cc.`运单编号` is not null AND cc.日期 >= '{1}' AND cc.日期 <= '{2}' AND cc.`是否改派` = '直发' AND cc.付款方式 in ({3})
                            ) cx
                            LEFT JOIN 
        ...
        sql14 = '''SELECT *
                FROM(SELECT IFNULL(s1.家族, '合计') 家族,IFNULL(s1.地区, '合计') 地区,IFNULL(s1.月份, '合计') 月份,
                    ...
                    FROM(SELECT IFNULL(cx.`家族`, '合计') 家族,
							...
							SUM(IF(cx.物流方式 = "台湾-速派-711超商" ,1,0)) AS 速派711超商总订单,
								SUM(IF(cx.物流方式 = "台湾-速派-711超商" AND 最终状态 = "已签收",1,0)) as 速派711超商已签收,
								SUM(IF(cx.物流方式 = "台湾-速派-711超商" AND 最终状态 = "拒收",1,0)) as 速派711超商拒收,
								SUM(IF(cx.物流方式 = "台湾-速派-711超商" AND 最终状态 = "已退货",1,0)) as 速派711超商已退货,
								SUM(IF(cx.物流方式 = "台湾-速派-711超商" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 速派711超商已完成,
							...
							SUM(IF(cx.物流方式 = "台湾-速派-黑猫" ,1,0)) AS 速派黑猫总订单,
								SUM(IF(cx.物流方式 = "台湾-速派-黑猫" AND 最终状态 = "已签收",1,0)) as 速派黑猫已签收,
								SUM(IF(cx.物流方式 = "台湾-速派-黑猫" AND 最终状态 = "拒收",1,0)) as 速派黑猫拒收,
								SUM(IF(cx.物流方式 = "台湾-速派-黑猫" AND 最终状态 = "已退货",1,0)) as 速派黑猫已退货,
								SUM(IF(cx.物流方式 = "台湾-速派-黑猫" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 速派黑猫已完成,
							SUM(IF(cx.物流方式 = "台湾-速派宅配通" ,1,0)) AS 速派宅配通总订单,
								SUM(IF(cx.物流方式 = "台湾-速派宅配通" AND 最终状态 = "已签收",1,0)) as 速派宅配通已签收,
								SUM(IF(cx.物流方式 = "台湾-速派宅配通" AND 最终状态 = "拒收",1,0)) as 速派宅配通拒收,
								SUM(IF(cx.物流方式 = "台湾-速派宅配通" AND 最终状态 = "已退货",1,0)) as 速派宅配通已退货,
								SUM(IF(cx.物流方式 = "台湾-速派宅配通" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 速派宅配通已完成,
							...
							SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹" ,1,0)) AS 铱熙无敌新竹总订单,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹" AND 最终状态 = "已签收",1,0)) as 铱熙无敌新竹已签收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹" AND 最终状态 = "拒收",1,0)) as 铱熙无敌新竹拒收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹" AND 最终状态 = "已退货",1,0)) as 铱熙无敌新竹已退货,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 铱熙无敌新竹已完成,
							SUM(IF(cx.物流方式 = "台湾-铱熙无敌-711超商" ,1,0)) AS 铱熙无敌711超商总订单,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-711超商" AND 最终状态 = "已签收",1,0)) as 铱熙无敌711超商已签收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-711超商" AND 最终状态 = "拒收",1,0)) as 铱熙无敌711超商拒收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-711超商" AND 最终状态 = "已退货",1,0)) as 铱熙无敌711超商已退货,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-711超商" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 铱熙无敌711超商已完成,
							SUM(IF(cx.物流方式 = "台湾-铱熙无敌-黑猫" ,1,0)) AS 铱熙无敌黑猫总订单,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-黑猫" AND 最终状态 = "已签收",1,0)) as 铱熙无敌黑猫已签收,
							...
							SUM(IF(cx.物流方式 = "速派新竹" ,1,0)) AS 速派新竹改派总订单,
								SUM(IF(cx.物流方式 = "速派新竹" AND 最终状态 = "已签收",1,0)) as 速派新竹改派已签收,
								SUM(IF(cx.物流方式 = "速派新竹" AND 最终状态 = "拒收",1,0)) as 速派新竹改派拒收,
								SUM(IF(cx.物流方式 = "速派新竹" AND 最终状态 = "已退货",1,0)) as 速派新竹改派已退货,
								SUM(IF(cx.物流方式 = "速派新竹" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 速派新竹改派已完成,
							SUM(IF(cx.物流方式 = "速派宅配通" ,1,0)) AS 速派宅配通改派总订单,
								SUM(IF(cx.物流方式 = "速派宅配通" AND 最终状态 = "已签收",1,0)) as 速派宅配通改派已签收,
								SUM(IF(cx.物流方式 = "速派宅配通" AND 最终状态 = "拒收",1,0)) as 速派宅配通改派拒收,
								SUM(IF(cx.物流方式 = "速派宅配通" AND 最终状态 = "已退货",1,0)) as 速派宅配通改派已退货,
								SUM(IF(cx.物流方式 = "速派宅配通" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 速派宅配通改派已完成,
							SUM(IF(cx.物流方式 = "速派黑猫" ,1,0)) AS 速派黑猫改派总订单,
								SUM(IF(cx.物流方式 = "速派黑猫" AND 最终状态 = "已签收",1,0)) as 速派黑猫改派已签收,
								SUM(IF(cx.物流方式 = "速派黑猫" AND 最终状态 = "拒收",1,0)) as 速派黑猫改派拒收,
								SUM(IF(cx.物流方式 = "速派黑猫" AND 最终状态 = "已退货",1,0)) as 速派黑猫改派已退货,
								SUM(IF(cx.物流方式 = "速派黑猫" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 速派黑猫改派已完成,
							...
							SUM(IF(cx.物流方式 = "天马黑猫" ,1,0)) AS 天马黑猫改派总订单,
								SUM(IF(cx.物流方式 = "天马黑猫" AND 最终状态 = "已签收",1,0)) as 天马黑猫改派已签收,
								SUM(IF(cx.物流方式 = "天马黑猫" AND 最终状态 = "拒收",1,0)) as 天马黑猫改派拒收,
								SUM(IF(cx.物流方式 = "天马黑猫" AND 最终状态 = "已退货",1,0)) as 天马黑猫改派已退货,
								SUM(IF(cx.物流方式 = "天马黑猫" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 天马黑猫改派已完成,
							SUM(IF(cx.物流方式 = "台湾-铱熙无敌-黑猫改派" ,1,0)) AS 铱熙无敌黑猫改派总订单,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-黑猫改派" AND 最终状态 = "已签收",1,0)) as 铱熙无敌黑猫改派已签收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-黑猫改派" AND 最终状态 = "拒收",1,0)) as 铱熙无敌黑猫改派拒收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-黑猫改派" AND 最终状态 = "已退货",1,0)) as 铱熙无敌黑猫改派已退货,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-黑猫改派" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 铱熙无敌黑猫改派已完成,
							SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹改派" ,1,0)) AS 铱熙无敌新竹改派总订单,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹改派" AND 最终状态 = "已签收",1,0)) as 铱熙无敌新竹改派已签收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹改派" AND 最终状态 = "拒收",1,0)) as 铱熙无敌新竹改派拒收,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹改派" AND 最终状态 = "已退货",1,0)) as 铱熙无敌新竹改派已退货,
								SUM(IF(cx.物流方式 = "台湾-铱熙无敌-新竹改派" AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 铱熙无敌新竹改派已完成
				            FROM (SELECT *, {4}
                                FROM {0}_zqsb cc where cc.`运单编号` is not null AND cc.日期 >= '{1}' AND cc.日期 <= '{2}' AND cc.付款方式 in ({3})
                            ) cx WHERE cx.`币种` = '台湾'
    ...
    def qsb_new(self, team, month_last):  # 报表各团队近两个月的物流数据
        ...
        sql10 = '''SELECT *
            FROM(SELECT IFNULL(月份, '总计') 月份,IFNULL(地区, '总计') 地区,IFNULL(家族, '总计') 家族,		
                ...
                ORDER BY 月份 DESC,
                        FIELD( 地区, '台湾', '香港', '总计' ),
                        FIELD( 家族, {2}),
                        总单量 DESC;'''.format(team, not_team, team_name2)
        ...
        sql50 = '''SELECT *, IF(神龙完成签收 = '0.00%' OR 神龙完成签收 IS NULL, 神龙完成签收, concat(ROUND(神龙完成签收-完成签收,2),'%')) as 神龙对比,
    			            ...
    			            IF(金蝉家族完成签收 = '0.00%' OR 金蝉家族完成签收 IS NULL, 金蝉家族完成签收, concat(ROUND(金蝉家族完成签收-完成签收,2),'%')) as 金蝉家族对比,
					...
					FROM(SELECT	IFNULL(月份, '总计') 月份, IFNULL(地区, '总计') 地区, IFNULL(产品id, '总计') 产品id, IFNULL(产品名称, '总计') 产品名称, IFNULL(父级分类, '总计') 父级分类,
                        ...
                        FROM(SELECT 年月 月份,币种 地区, 产品id, 产品名称, 父级分类,
                            ...
                            SUM(IF(家族 = '金蝉家族',1,0)) as 金蝉家族单量,
                                SUM(IF(家族 = '金蝉家族' AND 最终状态 = "已签收",1,0)) as 金蝉家族签收,
                                SUM(IF(家族 = '金蝉家族' AND 最终状态 = "拒收",1,0)) as 金蝉家族拒收,
                                SUM(IF(家族 = '金蝉家族' AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 金蝉家族完成,
                                SUM(IF(家族 = '金蝉家族' AND 是否改派 = '改派',1,0)) as 金蝉家族改派,
                            SUM(IF(家族 = '神龙-香港',1,0)) as 神龙运营单量,
                                SUM(IF(家族 = '神龙-香港' AND 最终状态 = "已签收",1,0)) as 神龙运营签收,
                                SUM(IF(家族 = '神龙-香港' AND 最终状态 = "拒收",1,0)) as 神龙运营拒收,
                                SUM(IF(家族 = '神龙-香港' AND 最终状态 IN ("已签收","拒收","已退货","理赔","自发头程丢件"),1,0)) as 神龙运营完成,
                                SUM(IF(家族 = '神龙-香港' AND 是否改派 = '改派',1,0)) as 神龙运营改派,
                            SUM(IF(家族 = '小虎队',1,0)) as 小虎队单量,
		...
		# 			WITH ROLLUP
		# 		) ss
        #         ORDER BY FIELD(ss.`家族`,'神龙','火凤凰','小虎队', '神龙香港','红杉','金狮','总计'),
        ...
        sql20 = '''SELECT *
                    ...
                    ORDER BY 月份 DESC,
                            FIELD( 地区, '台湾', '香港', '总计' ),
                            FIELD( s.家族, '神龙','火凤凰','小虎队','神龙香港','金蝉家族','奥创队','红杉','金狮', '总计' ),
    ...
    def address_repot(self, team, month_last, month_yesterday):    # 更新-地区签收率
        ...
        sql0 = '''SELECT s2.家族,s2.币种,s2.年月,s2.是否改派,s2.省洲,
                    ...
                    HAVING s2.年月 <> '合计'
        ORDER BY FIELD(s2.`家族`,'神龙','火凤凰','神龙香港','Line运营','金蝉家族','金蝉项目组','APP运营','神龙主页运营','小虎队','红杉','金狮','合计'),
                FIELD(s2.`币种`,'台湾','香港','合计'),
                s2.`年月`,
                FIELD(s2.`是否改派`,'改派','直发','合计'),
        ...
        sql1 = '''SELECT s2.家族,s2.币种,s2.年月,s2.是否改派,s2.省洲,
                            ...
                            FROM ( SELECT IFNULL(s1.币种,'合计') as 币种,
                                ...
                                FROM (SELECT cx.币种 as 币种,cx.家族 as 家族,cx.年月 as 年月,cx.是否改派 as 是否改派,cx.省洲 as 省洲,
                                            ...
                                            SUM(`价格RMB`) - SUM(IF(最终状态 = "未发货",`价格RMB`,0)) as 发货金额
                                        FROM (SELECT *,
                                                IF(cc.团队 LIKE "%红杉%","红杉",IF(cc.团队 LIKE "%火凤凰%","火凤凰",IF(cc.团队 LIKE "%神龙家族%","神龙",IF(cc.团队 LIKE "%金狮%","金狮",IF(cc.团队 LIKE "%金鹏%","小虎队",IF(cc.团队 LIKE "%神龙-香港%","神龙香港",cc.团队)))))) as 家族
                                            FROM {0}_zqsb cc where cc.`运单编号` is not null AND cc.日期 >= '{1}' AND cc.日期 <= '{2}'
                                        ) cx
    ...
    def phone_report(self, handle, month_last, month_yesterday):
        ...
        team_name = '''IF(cc.团队 LIKE "神龙-台湾%","神龙",
                    ...
                    '''
        team_name2 = '''
                        "神龙","火凤凰","金蝉家族优化组","金蝉项目组","金蝉家族公共团队","神龙香港","Line运营","APP运营","客服中心-港台""研发部-研发团队","神龙主页运营","奥创队","小虎队","红杉","金狮","合计"
                    '''
        logistics_name = '''
                            "台湾-森鸿-新竹-自发头程", "台湾-森鸿-新竹","台湾-大黄蜂普货头程-森鸿尾程","台湾-立邦普货头程-森鸿尾程",
                            "台湾-立邦普货头程-易速配尾程","台湾-大黄蜂普货头程-易速配尾程",  
        ...
        if week.isoweekday() == 1 or handle == '手动':
            ...
            sql = '''SELECT 日期31天,ss.问题订单,ss.正常出货,ss.删除订单,concat(ROUND(IFNULL(ss.删除订单/ss.问题订单,0) * 100,2),'%') as 取消占比,ss.实际签收, ss1.约派送,ss1.核实拒收 as 核实拒收原因,ss1.再派签收,ss2.挽回单数,ss2.未确认,ss2.退款单数,ss2.实际挽回单数,ss3.正常发货,ss3.取消订单,
                            ...
                            ss4.杨联系量 AS '杨嘉仪-联系量(有结果)',ss4.杨挽单量 AS '杨嘉仪-挽单量',  NULL 联系量,NULL 客户接听量,  NULL '张陈平-联系量',NULL '张陈平-客户接听量', NULL '蔡利英-联系量',NULL '蔡利英-客户接听量', NULL '杨嘉仪-联系量',NULL '杨嘉仪-客户接听量'
                    FROM date
                    LEFT JOIN
        ...
        if week.isoweekday() == 2 or week.isoweekday() == 4 or handle == '手动':
            ...
            sql = '''SELECT IFNULL(s.家族,'合计') as 家族, 
				    ...
				    WITH ROLLUP
				    HAVING s.币种 <> '合计';'''.format(month, time_bengin, time_end, team_name)  # 港台查询函数导出
