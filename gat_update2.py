...
class QueryUpdate(Settings):
    ...
    def EportOrder(self, team, month_last, month_yesterday, month_begin):
        ...
        if team in ('gat'):
            ...
            sql = '''SELECT 年月, 旬, 日期, 团队, 币种, null 区域, null 订单来源, a.订单编号, 电话号码, a.运单编号,
    ...
    # 新版签收率-报表(自己看的)
    def gat_new(self, team):  # 报表各团队近两个月的物流数据
        match = {'gat': '港台'}
    ...
    ORDER BY FIELD(s2.`家族`,'神龙','火凤凰','金狮','金鹏','红杉','合计'),
        ...
        sql18 = '''SELECT *
                ...
                ORDER BY FIELD(s.`家族`,'神龙','火凤凰','金狮','金鹏','红杉','合计'),
                        ...
                        FIELD(s.`产品id`,'合计'),
                        s.总订单 DESC;'''.format(team, month_last, month_yesterday)
        # df18 = pd.read_sql_query(sql=sql14, con=self.engine1)
        # listT.append(df18)

        today = datetime.date.today().strftime('%Y.%m.%d')
        sheet_name = ['物流分类', '物流分旬', '一级分旬', '二级分旬', '产品整月台湾', '产品整月香港', '产品分旬台湾', '产品分旬香港', '产品整月_直发台湾']
        ...
        sheet_name = ['物流分类', '物流分旬', '一级分旬', '二级分旬', '产品整月台湾', '产品分旬台湾', '产品整月香港', '产品分旬香港', '产品整月_直发台湾']
        df0 = pd.DataFrame([])  # 创建空的dataframe数据框
        df0.to_excel(file_path, index=False)  # 备用：可以向不同的sheet写入数据（创建新的工作表并进行写入）
        writer = pd.ExcelWriter(file_path, engine='openpyxl')  # 初始化写入对象
        book = load_workbook(file_path)  # 可以向不同的sheet写入数据（对现有工作表的追加）
        ...
        # self.e.send('{} 神龙-{}物流时效.xlsx'.format(today, tem1), filePath,emailAdd[tem1])
    ...
    def qsb_new(self, team):  # 报表各团队近两个月的物流数据
        ...
        sql14 = '''SELECT *,
                    ...
                    FROM(SELECT IFNULL(cx.`年月`, '总计') 月份,
                        ...
                        FROM (SELECT *,IF(cc.团队 LIKE "%红杉%","红杉",IF(cc.团队 LIKE "%火凤凰%","火凤凰",IF(cc.团队 LIKE "%神龙%","神龙",IF(cc.团队 LIKE "%金狮%","金狮",IF(cc.团队 LIKE "%金鹏%","金鹏",cc.团队))))) as 家族 
                            FROM gat_zqsb cc
                            where cc.日期 >= '2021-06-01' and cc.`运单编号` is not null 
                            ) cx
